...
> - **Enhance providers: 2 live** — Gemini and OpenAI (gpt-5 + image tool). **Grok was live for one day (2026-08-27 → 2026-08-28) and is dormant again** — it bled the fork red onto the machine body and re-posed the camera; the operator stopped rather than keep tuning. **BFL/Flux, Kontext and Reve were DELETED on 2026-08-27**, not parked — restoring one is a `git revert`, not a Literal edit. Ideogram is not a generator but is live for the per-variant Tweak and Inpaint tools. Any "4 providers" / "6 providers" text below is stale.
...
4. Optionally set the five visible toggles. They *append emphasis* to your prompt; they don't replace it — with one exception:
   - **Remove Background Entirely** is not a prompt at all. It runs a matting pass over the finished image and cuts the unit out with a real alpha channel, for the new-equipment site that shows units on no backdrop. Those images **export as transparent PNG with no disclaimer watermark**, since they go into a product-page composite. It overrides Perfect Showroom Floor. Since 2026-08-28 the engine is **fal.ai BiRefNet v2** rather than an in-container model; we ask fal for the mask and composite it onto our own pixels, so the machine's RGB is still never regenerated. **Writing "transparent background" or "white backdrop" into your prompt does not help and actively hurts** — a guardrail on every prompt forbids isolating the unit on a white/studio backdrop, and asking for it pushes the model into redrawing the machine's outline. If edges disappoint, the lever is `CUTOUT_FAL_MODEL` or `CUTOUT_OPERATING_RESOLUTION` (2304 is available), not prompt wording.
