...
3. **The differential scanner only sees the FIRST 1500 CHARACTERS of the operator's prompt.** `_describe_intended_edits` passes `custom_prompt.strip()[:1500]` as the record of what was deliberately requested. Everything past that character is, as far as the scanner is concerned, *not* an intended edit — and is therefore eligible to be reported as an anomaly. A long prompt does not just dilute adherence, it silently un-whitelists its own back half.
...
**Practical consequences for anyone writing or reviewing a prompt here:** keep the whole prompt under ~1500 characters so the scan whitelist covers it; treat a toggle as authoritative over prompt text on the same subject, and turn the toggle OFF rather than trying to argue with it in prose; and do not assume a built-in guardrail is protecting something on the prompt-first path without checking whether it sits inside the `spine_override is None` branch. **Decal / text preservation is one that does NOT get appended when a custom prompt is supplied** — it has to be in the operator's own text.
