...
18. **~~`_enhance_with_gemini` passes no `image_config`~~ — FIXED 2026-08-29, and the numbers are measured.** Same model, same input photo, same prompt, via `generate_content` (not AI Studio):
    ...
    ⚠️ **2K not 4K, and the reason is the container, not taste.** The API runs on **1 GiB with `Semaphore(8)`**. A 4836x3456 image decodes to ~50 MB of raw pixels for the resize, so eight concurrent jobs is 400 MB+ of pixel data alone, and a Cloud Run OOM kills every in-flight job rather than just the greedy one. 2K decodes to ~12.5 MB. **Raise the memory limit before setting `GEMINI_IMAGE_SIZE=4K`.** At x1.16 the residual upscale is nearly nothing, so 4K buys supersampling, not a fix.
