...
- **The matting engine is now an A/B toggle (2026-08-29).** `cutoutPhotoroom` picks Photoroom over fal BiRefNet; it is a no-op unless `transparentBackground` is also on, because it selects the ENGINE, it does not request a cutout. Everything after the mask arrives is identical for both vendors — same island filter, same local composite onto untouched RGB, same contract checks — which is what makes the comparison honest. `remove_background(..., engine=)` dispatches, and an **unknown engine raises rather than falling back to fal**: quietly matting with the wrong vendor would corrupt an A/B result instead of failing it.
  - ⚠️ **Photoroom's free tier is TEN IMAGES TOTAL and nothing meters it.** A batch of 8 with the toggle on spends 8. The count lives only in Photoroom's dashboard, and `PER_IMAGE_USD["photoroom/segment"]` is a **0.0 placeholder** — fix it before Photoroom carries real volume.
  - **`_mask_band` copes with either response shape and that is deliberate.** Photoroom's `channels=alpha` should give a bare mask, but the contract was written from vendor docs, not a live call (a call costs a credit). If a finished RGBA cutout arrives instead, its ALPHA is the mask — taking band 0 would use the RED channel as transparency and a black tyre would come back 92% see-through. It also guards the inverse: a greyscale mask saved with a fully-opaque alpha must use band 0, because that all-255 alpha would produce a completely opaque "cutout".
  - **The erase/tweak re-matte paths are pinned to `engine="fal"` on purpose.** Their payloads do not carry the enhance toggles, so they cannot know which vendor made the original. Tweaking a Photoroom cutout re-mattes it with fal — worth knowing while comparing.
...
**Auto Crop (`autoCrop`, 2026-08-29).** Locates the machine with Florence-2 on fal and reframes to the house 7:5 composition, so a set matches instead of some being tight and some wide. **PORTED FROM df-auto-edit (`c:\dev\df-auto-edit`), not reinvented** — that standalone crop tool targets the same 2800x2000 7:5 output for this same inventory, and its constants were MEASURED against real shoots. `services/autocrop.py` is a faithful port of its `lib/crop.ts` + `lib/detect/fal.ts`; read its `lib/config.ts` before touching a number here.

- ⚠️ **`AUTOCROP_PROMPT` is the single word `"forklift"` and that is measured, not lazy.** Over 121 real photos it located the machine in 100% of them — scissor lifts, telehandlers and boom lifts included, because the grounding is semantic rather than a class lookup. Expanding it to eight machine types made Florence-2 **LOSE** a forklift it had found at 75% coverage. **Do NOT substitute the request's `equipment_type`** — it is the obvious idea and the measurement says it is wrong.
- **Florence-2 request traps** (each cost df-auto-edit real time): the prompt field is `text_input`, NOT `text_prompt`; there is no `fal-ai/grounding-dino` (that slug 404s); boxes are **xywh in PIXELS of the image SENT**, not xyxy and not normalised; the response also carries a differently-sized annotated preview that does NOT describe those coordinates; and **no confidence scores are returned at all**, so every detection arrives at score 1 and df-auto-edit's `0.5*score` rank term is a constant here (size and centrality carry the decision).
- **`AUTOCROP_FILL = 0.88`** came from a visual sweep over 314 photos and REPLACED an earlier 0.78 that was correctly measured from an unrepresentative sample. Counter-intuitive part worth keeping: raising fill **reduces** clamping, because `cropW = subject / fill` — a bigger fill asks for a SMALLER crop, which is likelier to fit.
- **Vertical bounds are inviolable, horizontal ones are spendable.** Every forklift has forks and a viewer fills in a clipped tip without noticing; a cut-off mast top or missing wheels reads as a damaged or floating machine. So when the subject does not fit, the loss comes off the left/right edge, never the top/bottom. If no 7:5 window can hold the subject's height at all, it ships **uncropped** rather than clipping.
- **It runs BEFORE `upscale_to_standard`**, emitting a 7:5 rect so standardisation is a pure scale and the "every output is exactly 2800x2000" guarantee is untouched. ⚠️ **The cost of that placement is a re-upscale**: it crops the enhanced render and scales back up, so a heavy crop reintroduces some of the softness `GEMINI_IMAGE_SIZE=2K` just removed. The `sizing:` log reports the resulting factor on every image — watch it. The better long-term answer is to crop the SOURCE and let the vendor generate the tight framing natively; that was not done first because it touches the vendor input path, the stored original, and the differential scan's before/after pair.
- **A failure ships the image uncropped rather than failing the job**, unlike a cutout failure. An uncropped image is still a correct listing photo.
- One detection call per enhance JOB, so a 2-provider batch detects twice on the same source. `PER_IMAGE_USD["fal-ai/florence-2-large"]` is a **0.0 placeholder**.
...
18. **~~`_enhance_with_gemini` passes no `image_config`~~ — FIXED 2026-08-29, and the numbers are measured.** Same model, same input photo, same prompt, via `generate_content` (not AI Studio):
    | `image_size` | native output | pixels | what `upscale_to_standard` then does |
    |---|---|---|---|
    | *(unset)* | 1209x864 | 1.04 MP | cover-scale **x2.32** |
    | `"2K"` | 2418x1728 | 4.18 MP | cover-scale **x1.16** |
    | `"4K"` | 4836x3456 | 16.71 MP | cover-scale **x0.58** (a downsample) |

    So every stored asset used to be a **x2.32 lanczos upscale of a 1 MP generation** — that is the "blurry, looks like JPEG artifacts" the operator reported, and it was always there; a transparent cutout just removed the busy background that had been hiding it. `GEMINI_IMAGE_SIZE` (default **`2K`**) now sets it, shared by enhance AND tweak via `_gemini_generate_image` — tweak matters because it also runs `upscale_to_standard`, so leaving it at the default would have re-blurred any variant the operator tweaked. Aspect ratio needed nothing: Gemini already returns 1.3993 against the standard's 1.4000, so `_cover_crop` was never the problem.

    ⚠️ **2K ALONE OOM'd the container, within minutes of shipping.** I predicted the memory ceiling would bite at 4K and was wrong about the margin: at 1 GiB / 1 CPU, 2K produced four `Out-of-memory event detected in container` + `Container terminated on signal 9` kills in 16 minutes. SIGKILL takes every in-flight job with it, which is what "only 2 of a batch of 8 got a cutout" and a screen of 503s actually were — not a fal concurrency limit. **The decoded image is a small part of the footprint**: the real cost is the whole pipeline's transient buffers (adjust, cover-scale, the matting upload and its base64 copy, the composite), which is why per-image arithmetic underestimated it. Two fixes landed together: the container went to **`--cpu=2 --memory=4Gi`**, and the fal upload stopped being PNG (see the matting section). Cloud Tasks allows 10 concurrent dispatches and `--concurrency=80` lets them all land on ONE instance, so size for ten heavy jobs, not one. **Do not try `GEMINI_IMAGE_SIZE=4K` without re-measuring memory** — 4K is 4x these buffers again. At x1.16 the residual upscale is nearly nothing, so 4K buys supersampling, not a fix.

    The call **falls back** to the model default if `image_config` is ever rejected, because a preview model can drop a field between revisions and soft output beats every enhance job failing. `upscale_to_standard` now logs `sizing: vendor returned WxH (N MP), cover-scaling xF` on every path, so this stops being a matter of opinion. ⚠️ `PER_IMAGE_USD` still carries a flat **$0.039** for this model; Google prices image output by tokens, which scale with resolution, so that row is probably now low — check it against a real bill.
