...
| `recraft` | `recraftv3` at `https://external.api.recraft.ai/v1/images/imageToImage` | **Sync** multipart endpoint. Bearer auth. Body matches docs exactly: `image` (file) + `prompt` + `style=realistic_image` + `strength=0.45`. NO `model` or `n` — imageToImage implicitly uses V3 (V4.1 is generations-only); extras cause no harm but aren't in the spec. Response JSON: `data[0].url` or `image.url` depending on endpoint version — `_enhance_with_recraft` defensively unwraps either shape. **Prompt cap: 1000 UTF-8 bytes** — vendor returns `invalid_request_parameter` over that. Stopgap byte-safe truncation at 990 bytes lives in `_enhance_with_recraft` until `_build_recraft_prompt` (per-model prompts work item) lands. Product-photography-tuned model; distinct "polished marketing-stock" voice vs. the more general-creative providers. Wired + secret-rotated + truncation-tested end-to-end 2026-05-26. | `cleanshot-recraft-key` |
...
1. **Per-model enhance prompts.** Single biggest quality lever left. Current `_build_enhance_prompt` is ~200 lines of declarative scene prose tuned for Gemini Nano Banana edit semantics, and it ships verbatim to every provider. Kontext especially is mismatched — BFL positions it for short imperative prompts (1–3 sentences) to preserve subject identity; long prose dilutes that. Recraft is similarly mismatched AND has a hard 1000-byte cap, so `_enhance_with_recraft` currently byte-truncates at 990 — output is functional but ugly because the truncation chops off the equipment-anatomy guardrails and action checklists in the back half of the prompt. Discussed plan: add `_build_kontext_prompt` first (Phase A, smallest win), then `_build_recraft_prompt` (replaces the truncation stopgap, unlocks Recraft quality), then `_build_openai_prompt` + eval harness (Phase B). Grok can share OpenAI's style. **When `_build_recraft_prompt` lands, delete the byte-truncation block in `_enhance_with_recraft` — the new builder should produce text safely under 1000 bytes by design.**
